...
public class MessageServiceTest {
	...
	public void sendPremiumExpiryMessageTest() {
		User user = new User();
		user.setUsername("expiry@test.com");
		user.setEmail("expiry@test.com");
		user.setFirstName("Expiry");
		user.setLastName("Test");
		user.setPassword("123123");
		user.setEnabled(true);
		user.setGender(Gender.FEMALE);
		user.setAccountType(AccountType.PREMIUM);
		user.setCreditCard("1234123412341234");

		user.setCreditcardType(CreditcardType.AMEX);
		user.setSecurityNumber("123");
		user.setExpirationMonth("01");
		user.setExpirationYear("2018");
		user.setCreditcardName("Expiry Test");
		...
		assertEquals(0, ListUtils.countIterable(messageService.getInboxForUser(user)));
		
		messageService.sendPremiumExpiryMessage();
		
		List<Message> messages = ListUtils.convertToList(messageService.getInboxForUser(user));
		
		assertEquals(1, messages.size());
		
		assertEquals("Your Ithaca premium membership will expire tomorrow!", messages.get(0).getSubject());
	}
